...
type Product = {
  ...
  highlight: string
  nameOnServer: string
  colorOnServer: string
  commandOnServer: string
  position: string
  expiresTime: string
}
...
export function UpdateForm(props: any) {
  const { useLoading, UpdateProduct, DeleteProductImg } =
    useContext(ProductsContext)
  const [product] = useState<Product>(props.props.product)
  const [images, setImages] = useState<any>(props.props.images)
  const [files, setFiles] = useState<any>([])
  const [todo, setTodo] = useState<any>(props.props.todo)
  ...
  async function handleImgDelete(key: string) {
    try {
      const img = key.split('/')
      const response = await DeleteProductImg(product?.id, img[6])
      if (response.status === 200) {
        setImages(images.filter((img: string) => img !== key))
      }
    } catch (err) {
      console.log(err)
  ...
  async function handleUpdate(data: any) {
    try {
      await UpdateProduct(product?.id, data, todo, files)
    } catch (err) {
      AlertError('Ocorreu um erro na requisição!')
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(handleUpdate)}>
        ...
        <div className="w-full lg:w-1/2">
          <div className="mt-3">
            ...
            <input
              ...
              defaultValue={product?.nameOnServer}
              type="text"
              name="nameOnServer"
          ...
          <div className="mt-3">
            ...
            <input
              ...
              defaultValue={product?.colorOnServer}
              type="text"
              name="colorOnServer"
          ...
          <div className="mt-3">
            ...
            <input
              ...
              defaultValue={product?.commandOnServer}
              type="text"
          ...
          <div className="mt-3">
            ...
            <input
              ...
              defaultValue={product?.expiresTime}
              type="text"
              name="expiresTime"
      ...
      <div className="grid gap-4 mt-5 lg:grid-cols-8">
        {images.map((img: string, index: any) => {
          return (
            <div className="h-32 relative w-32" key={index}>
              <a
                onClick={() => handleImgDelete(img)}
                className="bg-black rounded cursor-pointer text-xl right-0 top-0 focus:outline-none p-2 absolute text-color-danger"
              >
                <FaTrashAlt />
              </a>
              <img
                className="rounded-md h-32 object-cover w-32"
                src={img}
                alt="Product Image"
              />
            </div>
