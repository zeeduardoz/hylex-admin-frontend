...
type OrdersContextType = {
  ...
  FilterOrders: (data: any) => Promise<any>
}
...
export const OrdersProvider: React.FC = ({ children }: any) => {
  ...
  async function GetOrders() {
    ...
    const response = await api.get(
      `/admin/purchases/list?perPage=${5}&currentPage=${page}`,
  ...
  async function GetOrdersNext() {
    ...
    const response = await api.get(
      `/admin/purchases/list?perPage=${5}&currentPage=${page + 1}`,
      { headers: { Authorization: 'Bearer ' + token } }
    )
  ...
  async function FilterOrders(data: any) {
    const { '@hylex/token': token } = parseCookies()

    setLoading(true)

    const response = await api.get(
      `/admin/purchases/listByNickname?Nickname=${data}`,
      {
        headers: { Authorization: 'Bearer ' + token }
      }
    )
